...
export class ApiService {
    ...
    async getTracks(): Promise<IFile[]> {
        ...
        rawPaths.forEach(path => {
            ...
            const isMedia = category === 'media' || category === 'fotos' || category === 'videos';
    ...
    async getTrackGeoJSON(filename: string): Promise<any> {
        const response = await axios.get(`${this.baseUrl}/track/${filename}`);
        return response.data;
    }

    async uploadFiles(files: FileList, category: string, location?: { lat: number, lng: number }): Promise<void> {
    ...
    async updateMediaLocation(filename: string, latitude: number, longitude: number): Promise<void> {
        try {
            console.log('[ApiService] Updating location for:', filename, 'to', latitude, longitude);
            const response = await axios.post(`${this.baseUrl}/media/${filename}/location`, {
                latitude,
                longitude
            });
