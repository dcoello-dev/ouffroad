...
const buildTree = (files: IFile[]): Record<string, TreeNode> => {
    ...
    files.forEach(file => {
        ...
        if (dirs.length === 0) {
            ...
            if (!root[category]) {
                root[category] = { name: category, path: category, children: {}, files: [], isOpen: true };
            }
...
const TreeNodeComponent: React.FC<TreeNodeProps> = ({ node, activeFiles, onToggle, onSetLocationRequest, level }) => {
    ...
    const headerClass = isCategory ? 'category-header' : (level === 1 ? 'year-header' : 'month-header');
    const containerClass = isCategory ? 'category-group' : (level === 1 ? 'year-group' : 'month-group');
    ...
    const collectFiles = (n: TreeNode): IFile[] => {
        let files = [...n.files];
        Object.values(n.children).forEach(child => files.push(...collectFiles(child)));
        return files;
    };

    const allNodeFiles = collectFiles(node);
    const allActive = allNodeFiles.every(f => activeFiles.some(af => af.fullPath === f.fullPath));
    const someActive = allNodeFiles.some(f => activeFiles.some(af => af.fullPath === f.fullPath));
        ...
        <div className={containerClass}>
            ...
            {isOpen && (
                <div className="group-content">
                    ...
                    {Object.values(node.children).sort((a, b) => b.name.localeCompare(a.name)).map(child => (
                        <TreeNodeComponent
                            key={child.path}
                            node={child}
                    ...
                    {node.files.map(file => {
                        const isActive = activeFiles.some(f => f.fullPath === file.fullPath);
                        ...
                        return (
                            <div
                                ...
                                style={{ marginLeft: `${(level + 1) * 15}px`, display: 'flex', alignItems: 'center' }}
                            >
                                <input
                                    type="checkbox"
                                    checked={isActive}
                                ...
                                />
                                <span style={{ flex: 1 }}>{file.filename.split('/').pop()}</span>
                                {isMedia && (
                                    <>
                                        <button
                                            onClick={(e) => {
                                                ...
                                                window.open(`${repoBaseUrl}/${file.fullPath}`, '_blank');
                                            }}
...
export const Sidebar: React.FC<SidebarProps> = ({ files, activeFiles, onToggle, onUploadComplete, onSetLocationRequest }) => {
