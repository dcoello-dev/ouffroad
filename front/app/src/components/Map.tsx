...
const MapClickHandler = ({ pickingLocationFor, onLocationPicked }: { pickingLocationFor: string | null, onLocationPicked: (lat: number, lng: number) => void }) => {
    const map = useMap();
...
const GeoJsonLayer = ({ file }: { file: IFile }) => {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            try {
                ...
                const geojson = await ApiService.getInstance().getTrackGeoJSON(file.fullPath);
    ...
    if (file.type === 'media') {
        ...
        console.log('[Map] Media URL constructed:', mediaUrl, 'from repoBaseUrl:', repoBaseUrl, 'and fullPath:', file.fullPath);
                    ...
                    <div style={{ textAlign: 'center' }}>
                        ...
                        {mediaFile.isVideo() ? (
                            ...
                            <a href={mediaUrl} target="_blank" rel="noreferrer">
                                <img src={mediaUrl} style={{ width: '100%' }} alt={file.filename} />
                            </a>
                        )}
                        <button
    ...
    return <GeoJSON data={data} style={{ color: getColor(file.category), weight: 2 }} />;
};

function getColor(category?: string): string {
    switch (category) {
        case 'enduro': return 'red';
        case 'trail': return 'gold';
        default: return 'blue';
    }
}

export const MapComponent: React.FC<MapProps> = ({ activeFiles, pickingLocationFor, onLocationPicked }) => {
    return (
        <MapContainer center={[0, 0]} zoom={2} style={{ height: '100%', width: '100%' }}>
            <MapClickHandler pickingLocationFor={pickingLocationFor} onLocationPicked={onLocationPicked} />
